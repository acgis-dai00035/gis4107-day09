...
def conv_num_with_commas(num_text):
...
def set_country_populations_dict():
    ...
    for i in range(1,get_country_count()+1):
     country_pop_dict[lines[i].split('\t')[1]] = (conv_num_with_commas(lines[i].split('\t')[5]),lines[i].split('\t')[6] )
...
def get_population(country_name):
    ...
    if country_pop_dict.has_key(country_name) :
    ...
    return country_pop_dict[country_name][0]
...
def get_continent_populations():
    ...
    for k in range(1,get_country_count()+1):
        ...
        dict2[lines[k].split('\t')[2]] += conv_num_with_commas(lines[k].split('\t')[5])
